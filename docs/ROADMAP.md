...
Status as of **v0.4.0**. Hardware verification on **board1** (ProMicro clone,
app @ **`0x1000`**, J-Link + COM18) unless noted. **Seeed XIAO nRF52840**:
compile-only locally until HW is available.

## Done (shipped)

| Area | Delivered |
|------|-----------|
| **Core API** | `random`, SHA-256/384/512, HMAC-SHA-256, HKDF-SHA-256, AES-CBC/CTR/GCM, ChaCha20-Poly1305, ECDSA/ECDH P-256, **X25519**, **Ed25519**, **RSA-2048 PKCS#1 SHA-256** (+ pub export / VerifyPub) |
...
| Priority | Item | Notes |
|----------|------|-------|
| Ops | **XIAO HW smoke** | Local compile-only today; HW when board available |

## Explicit non-goals (for now)

- Redistributing Nordic binaries in git or Library Manager
- GitHub Actions / remote CI automation (local compile + board1 HW verify only)
...
| v0.3.0 | SHA-512, SdCryptoSmoke, roadmap |
| v0.3.1 | Library Manager indexing, SHA-512 KAT fix |
| v0.3.2 | SHA-384, HKDF-SHA-256, shim SHA-512, doc sync |
...
| v0.4.0 | Ed25519, RSA pub export/VerifyPub, BleCryptoStress NUS GATT path |
